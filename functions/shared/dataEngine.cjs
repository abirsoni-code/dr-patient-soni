...
async function handleAction(store, action, params) {
  ...
  switch (action) {
    ...
    case "register": {
      // `uid` is a Firebase Auth UID, set by functions/index.js from a verified
      // ID token — never trust a client-supplied uid directly. No password is
      // stored here; Firebase Auth owns credentials entirely.
      const { uid, role, name, surname, address, mobile, photoUrl, username, city, area, opdTimings, specialization } = params;
      if (!uid || !role || !name || !mobile || !username) {
      ...
      if (data.users.some((u) => u.userId === uid)) {
        throw new Error("Profile already exists for this account");
      }
      const user = {
        userId: uid, role, name, surname: surname || "", address: address || "",
        mobile, photoUrl: photoUrl || "", username, createdAt: nowIso(),
    ...
    case "getProfile": {
      // `uid` is a Firebase Auth UID, set by functions/index.js from a verified
      // ID token — this replaces password-based login entirely.
      const { uid, role } = params;
      const user = data.users.find((u) => u.userId === uid && u.role === role);
      if (!user) throw new Error("No profile found for this account");
      return user;
    }
