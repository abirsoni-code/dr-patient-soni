 ...
 * ============================================================================
 * Backend: a small JSON-file "database", served two ways from the same
 * shared/dataEngine.cjs logic:
 *
 *   - LOCAL DEV:  `npm run server` runs an Express server that reads/writes
 *                 server/data/db.json. Vite proxies /api to it (see
 *                 vite.config.js). This is your "local sheet."
 *
 *   - NETLIFY:    netlify/functions/api.js runs the same logic using
 *                 Netlify Blobs for storage, reachable at /api once deployed
 *                 (see netlify.toml for the redirect). No setup needed —
 *                 it works automatically on Netlify, nothing to configure.
 *
 * Both are called through the same relative URL, "/api", so this file
 * doesn't need to change between local dev and production.
 *
 * If API_URL is left blank AND no /api server responds, the app falls back
 * to in-memory DEMO MODE automatically so it's never broken.
 * ============================================================================
 */
...
// ---------------------------------------------------------------------------
// Mock backend (used only when API_URL is empty, for demo purposes)
// ---------------------------------------------------------------------------
const mockDb = {
  users: [
    {
      userId: "doc-1", role: "Doctor", name: "Alina", surname: "Kessler",
    ...
    },
    {
      ...
      address: "204 Baner Road, Pune", mobile: "9876500002", photoUrl: "",
      username: "rohan.m", password: "demo123",
    },
  ],
  appointments: [
    {
      appointmentId: "appt-1", patientId: "pat-1", doctorId: "doc-1",
      patientName: "Rohan Mehta", doctorName: "Dr. Alina Kessler",
      symptoms: "Persistent dry cough, mild fever for 2 days",
      date: todayISO(), time: "10:30", status: "Confirmed",
    },
    {
      appointmentId: "appt-2", patientId: "pat-1", doctorId: "doc-1",
      patientName: "Rohan Mehta", doctorName: "Dr. Alina Kessler",
      symptoms: "Follow-up: blood pressure check",
      date: todayISO(), time: "15:00", status: "Confirmed",
    },
  ],
};
...
const mockPhotos = {}; // prescriptionId -> { photoBase64, contentType }

async function mockCall(action, params) {
  ...
  switch (action) {
    ...
    case "bookAppointment": {
      ...
      const appt = { appointmentId, patientId, doctorId, patientName, doctorName, symptoms, date, time, status: "Confirmed" };
    ...
    case "rescheduleAppointment": {
      ...
      if (hoursUntil < 24) throw new Error("Cannot reschedule within 24 hours of the appointment");
      appt.date = date;
      appt.time = time;
    ...
    case "uploadPrescription": {
      ...
      mockPhotos[prescriptionId] = { photoBase64, contentType: meta.contentType };
      return meta;
    }
...
// ---------------------------------------------------------------------------
// API layer — talks to the local/Netlify backend at /api, falling back to
// in-memory mock data automatically if that backend isn't reachable
// (e.g. you're only running `npm run dev` without `npm run server`).
// ---------------------------------------------------------------------------
const READ_ACTIONS = new Set(["listDoctors", "doctorAppointments", "patientAppointments", "ping", "listPrescriptions", "getPrescriptionPhoto"]);
...
async function probeBackend() {
  ...
  try {
    const res = await fetch(`${API_URL}?action=ping`, { signal: AbortSignal.timeout(2500) });
    const json = await res.json();
    backendReachable = !!json.ok;
  } catch {
...
async function realCall(action, params) {
...
async function api(action, params = {}) {
  ...
  if (!reachable) return mockCall(action, params);
...
function RoleSelectScreen({ onSelect }) {
  const [usingMock, setUsingMock] = useState(null); // null = checking, true/false once known

  useEffect(() => {
    probeBackend().then((reachable) => setUsingMock(!reachable));
  }, []);
  ...
  return (
    <div style={{ padding: "0 24px" }}>
      ...
      {usingMock === true && (
        <div
          style={{
            marginTop: 28, fontSize: 12.5, color: COLORS.tealDeep, background: COLORS.parchmentDim,
            border: `1px dashed ${COLORS.line}`, borderRadius: 10, padding: "12px 14px", lineHeight: 1.6,
          }}
        >
          <strong>Demo mode</strong> — no backend reachable at <code style={{ background: COLORS.white, padding: "1px 5px", borderRadius: 4 }}>/api</code>, running on in-memory mock data instead.
          Run <code style={{ background: COLORS.white, padding: "1px 5px", borderRadius: 4 }}>npm run server</code> locally, or deploy to Netlify, to use real persisted data.
          <br />
          Try it: Doctor login <code>dr.kessler</code> / <code>demo123</code> · Patient login <code>rohan.m</code> / <code>demo123</code>
        </div>
      )}
      {usingMock === false && (
        <div
...
function EditProfileScreen({ user, onBack, onSaved }) {
  ...
  async function submit(e) {
    ...
    setLoading(true);
    try {
      const updated = await api("updateProfile", { userId: user.userId, ...form });
...
function DoctorDashboard({ user, onLogout, onEditProfile }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await api("doctorAppointments", { doctorId: user.userId, date });
...
function PrescriptionSection({ appointmentId, currentUserId }) {
  ...
  async function remove(item) {
    try {
      await api("deletePrescription", { prescriptionId: item.prescriptionId, requesterId: currentUserId });
...
function PatientDashboard({ user, onLogout, onEditProfile }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await api("patientAppointments", { patientId: user.userId });
      setAppts(data.map(toApptView));
    } catch (err) {
      setError(err.message);
    } finally {
...
function AppointmentsList({ appts, loading, onReschedule, onCancelled, setError, currentUserId }) {
  ...
  async function cancel(appt) {
    ...
    try {
      await api("cancelAppointment", { appointmentId: appt.AppointmentID });
      onCancelled();
    } catch (err) {
      setError(err.message);
    } finally {
...
function BookingFlow({ user, rescheduling, onCancelReschedule, onBooked }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const existing = await api("doctorAppointments", { doctorId, date });
        setBookedTimes(
          existing
            .filter((a) => !rescheduling || a.appointmentId !== rescheduling.AppointmentID)
            .map((a) => a.time)
        );
      } catch {
        setBookedTimes([]);
  ...
  async function submit() {
    ...
    try {
      if (rescheduling) {
        const updated = await api("rescheduleAppointment", { appointmentId: rescheduling.AppointmentID, date, time });
        onBooked({
